...
class EBDLessonSerializer(serializers.ModelSerializer):
    ...
    def get_presence_records(self, obj):
        return {
            ...
            'pending_calls': EBDPresenceRecord.objects.filter(lesson__pk=obj.pk, attended=False).values('ebd_class').annotate(count=Count('id'), unique_names=Count('ebd_class', distinct=True))
        }
