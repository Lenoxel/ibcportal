...
class EBDLessonSerializer(serializers.ModelSerializer):
    ...
    def get_presence_records(self, obj):
        return {
            ...
            'pending_calls': EBDPresenceRecord.objects.filter(lesson__pk=obj.pk, attended=False).values('ebd_class__name').annotate(count=Count('ebd_class__id', distinct=True))
        }
