...
def create_audit(responsible, changed_model, action_type, obj_name):
    description = 'O administrador {} realizou a ação de {} em "{}" ({}).'.format(
        responsible, action_type, obj_name, changed_model
    )
    audit_object = {
    ...
    if len(all_devices) > 0:
        ...
        if len(valid_registration_ids) > 0:
            ...
            if entity_type == "video":
                ...
                if start_date.day == datetime.now().astimezone(
                    get_localzone()
                ).day and end_date > datetime.now().astimezone(get_localzone()):
                    if start_date > now:
                        message_title = "Tem evento logo mais!"
                        message_body = (
                            'Se liga nesse evento que vai acontecer hoje: "'
                            + title
                            + '". Fica ligado, pois vai começar às '
                        ...
                        message_body = (
                            'Se liga nesse evento que está acontecedo: "' + title + '".'
                        )
                ...
                if published_date is None or (
                    published_date is not None
                    and published_date <= datetime.now().astimezone(get_localzone())
                ):
                    message_title = "Postagem nova no app!"
                    if form.cleaned_data["publisher"].nickname is not None:
                        message_body = (
                            form.cleaned_data["publisher"].nickname
                            + ' acabou de fazer uma postagem: "'
                            + title
                            + '". Confere lá!'
                        )
                    else:
                        message_body = (
                            form.cleaned_data["publisher"].name
                            + ' acabou de fazer uma postagem: "'
                            + title
                            + '". Confere lá!'
                        )
                ...
                if start_date.day == datetime.now().astimezone(get_localzone()).day:
                    ...
                    if form.cleaned_data["title"] is not None:
                        message_body = (
                            "Fique ligado, pois hoje teremos "
                            + meeting_types.get(form.cleaned_data["title"])
                            + " às "
                            + start_date.strftime("%H:%M")
                            + "."
                        )
                        ...
                        if form.cleaned_data["organizing_group"] is not None:
                            message_body = (
                                "Fique ligado, pois hoje teremos Programação - "
                                + form.cleaned_data["organizing_group"].name
                                + " às "
                                + start_date.strftime("%H:%M")
                                + "."
                            )
                            ...
                            message_body = (
                                "Fique ligado, pois hoje teremos Programação Geral às"
                                + start_date.strftime("%H:%M")
                                + "."
                            )
            ...
            if result is not None:
                save_push_notification_info(
                    message_title, message_body, result, push_date
                )
...
def get_sunday(sundays_before_index: int):
    ...
    sunday_date = (
        ...
        if today.weekday() == 6 and sundays_before_index == 0
        else (
            today - (timedelta(today.weekday() + 1 + (sundays_before_index * 7)))
        ).strftime("%d/%m/%Y")
    )
    return sunday_date
...
def remove_person_from_old_ebd_classes(person: Member, current_ebd_class_id: int):
    ...
    ebd_classes_to_remove_person = (
        ebd_class for ebd_class in ebd_classes if ebd_class.pk != current_ebd_class_id
    )
