...
from core.models import Event, NotificationDevice, PushNotification
        ...
        if len(events) > 0:
            ...
            if len(all_devices) > 0:
                ...
                if len(valid_registration_ids) > 0:
                    if len(events) == 1:
                        message_title = "Ei! Dá uma olhada nesse evento"
                        message_body = (
                            'Não perde não... Hoje vai rolar o "' + events[0] + '".'
                        )
                    else:
                    ...
                    data_message = {"entity_type": "event", "redirect": True}
